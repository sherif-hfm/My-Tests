...
namespace OneTimePassApi
{
    ...
    public class OneTimePassword
    {
        ...
        public bool IsCodeValid(int code)
        {
            ...
            var nextCode = GetCode(this.Counter + 1, this.Digits);
            ...
            var isNextValid = (code == nextCode) && (this.Counter > 0); //don't check previous code if counter is zero; but calculate it anyhow (to keep timing)
            var isValid = isCurrValid || isPrevValid || isNextValid;
        ...
        public static void FromBase32(string text, byte[] buffer, out int length)
        {
            var index = 0;
        ...
        public static string ToBase32(byte[] bytes, int length, SecretFormatFlags format)
        {
